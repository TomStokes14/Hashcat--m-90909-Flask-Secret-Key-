...
KERNEL_FQ void m90909_mxx (KERN_ATTR_BASIC ())
{
  /**
   * modifier
   */
...
/*
    ...
    sha1_hmac_ctx_t ctx;
    sha1_hmac_init_swap (&ctx, c, pw_len + comb_len);     // chris guesses swap
    sha1_hmac_update (&ctx, fixed, 14);
    sha1_hmac_final (&ctx);
    // We do need to copy to a temporary array because the ctx.opad.h is only size 5
...
KERNEL_FQ void m90909_sxx (KERN_ATTR_BASIC ())
{
  ...
  for (u32 il_pos = 0; il_pos < IL_CNT; il_pos++)
  {
    ...
    sha1_hmac_ctx_t ctx;
    sha1_hmac_init (&ctx, c, pw_len + comb_len);     // chris guesses swap
    sha1_hmac_update (&ctx, fixed, 14);
    sha1_hmac_final (&ctx);
    // We do need to copy to a temporary array because the ctx.opad.h is only size 5
    u32 intermediate[16] = {0};
